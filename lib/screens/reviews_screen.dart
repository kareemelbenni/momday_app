...
class _ReviewsScreenState extends State<ReviewsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final goIcon = Localizations.localeOf(context).languageCode == 'ar'? Icons.keyboard_arrow_left : Icons.keyboard_arrow_right;

    final alreadyReviewed
      = widget.product.reviews.any((review) => review.customerId == AppStateManager.of(context).account?.customerId);
        ...
        Padding(
          ...
          child: ListView(
            ...
            children: <Widget>[
              ...
              this._buildMainView(context)
            )..addAll(
              this.mode != ReviewMode.Write && !alreadyReviewed? [
                Container(
  ...
  List<Widget> _buildCurrentReview(BuildContext context, ReviewModel currentReview) {
    return [
      SizedBox(height: 16.0,),
      Container(
        foregroundDecoration: BoxDecoration(
...
typedef WriteReviewCallback(String name, String reviewText, double reviewRating);
...
class _WriteReviewWidgetState extends State<WriteReviewWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    if(AppStateManager.of(context).account != null && AppStateManager.of(context).account.fullName != null)
            ...
            onPressed: () async {
              if (!this._isLoading) {
                if (this._formKey.currentState.validate()) {
                  ...
                  await widget.onWriteReview(this._name, this._reviewText, this._rating);
