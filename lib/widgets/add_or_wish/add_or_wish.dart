...
class AddOrWishState extends State<AddOrWish> {
  ...
  Widget _getActionButton({BuildContext context, bool status, bool isPerformingAction, IconData containedIcon, IconData notContainedIcon, VoidCallback onPressed, bool isFirst}) {
    ...
    final icon = Icon(
      status? containedIcon : notContainedIcon
    );
    ...
    return Container(
      ...
      foregroundDecoration: BoxDecoration(
        border: BorderDirectional(
          start: !isFirst || this.widget.hasStartBorder? BorderSide(
            width: 0.1
          ) : BorderSide.none,
          top: BorderSide(
              width: 0.1
          ...
          end: isFirst? BorderSide.none : BorderSide(
              width: 0.1
          ),
        ),
...
//        .any((element) => element.productId == this.widget.product.productId);
  ...
  _handleCartButton(bool containedInCart) async {
    ...
    if(this.widget.product.availableOptions != null && this.widget.product.availableOptions.length > 0 && this.widget.fromListing) {
      displayOptionsScreen();
    }
      ...
      if(this.widget.newItem) {
        for (OptionModel option in this.selectedOptions.keys)
          if (this.selectedOptions[option] == null) {
            showTextSnackBar(context, "${tSentence(context, 'select_option')} ${option.name}");
            missingFields = true;
            break;
          }
      }
      ...
      if(this.widget.quantity > maxQuantity()){
        showTextSnackBar(context, "not enough quantity for the selected options");
      }
