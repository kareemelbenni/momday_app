...
class ExistingAddressesState extends State<ExistingAddresses> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.addresses.length > 0) {
      children = widget.addresses.expand((address) =>
      [
        Container(
          foregroundDecoration: BoxDecoration(
              border: Border.all(
                  color: address == this.selectedAddress? MomdayColors.MomdayGold : Colors.black,
                  width: 2.0
              )
          ),
          child: ListTile(
            ...
            trailing: IconButton(
                ...
                onPressed: () async {
                  final bool addressModified = await Navigator.of(context, rootNavigator: true).push(
                      MaterialPageRoute(builder: (context) => ManageAddressScreen(addressModel: address,))
                  );

                  if (addressModified && widget.onAddressesModified != null) {
        ...
        SizedBox(height: address == widget.addresses.last? 0.0 : 8.0,)
      ]
      ).toList();
    } else {
      children = [
        Text(
          tSentence(context, 'no_addresses'),
